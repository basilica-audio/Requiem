...
TEST_CASE ("PresetManager: current and legacy preset folders follow the platform convention", "[presets][branding]")
{
    ...
    REQUIRE (current == presetsRoot.getChildFile ("Basilica Audio").getChildFile ("Requiem"));
    REQUIRE (legacy == presetsRoot.getChildFile ("Yves Vogl").getChildFile ("Requiem"));
   #else
    ...
    REQUIRE (current == applicationData.getChildFile ("Basilica Audio")
                            .getChildFile ("Requiem").getChildFile ("Presets"));
