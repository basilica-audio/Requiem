...
Requiem is a cinematic **convolution reverb** built for orchestral and choral space in heavy productions - cathedral, hall, and chamber-style tails for strings, choir, and everything that needs to bloom into a wide, dark stereo image behind the wall of guitars. Rather than shipping (and licensing) an IR sample library, its impulse response is generated **procedurally**, off the audio thread, from Decay, Damping, Space, Size, Early/Late Balance, and Bass Decay - or you can load your own WAV/AIFF impulse response to override it entirely. The early-reflection and tail-darkening behavior is a research-derived rework (v0.2.0) against documented cinematic-reverb design principles - see [`docs/design-brief.md`](docs/design-brief.md)/[`docs/research-notes.md`](docs/research-notes.md) for the full sourcing.

## Features

- `juce::dsp::Convolution`-based stereo reverb engine with a **multiband** decay: independent low/mid/high RT60 rates plus a progressively darkening high band, driven by **Decay** (reverb time) and **Damping** (the tail's terminal HF corner) - no bundled sample library, no licensing to manage.
- **Space** (Cathedral/Hall/Chamber) shapes a density-buildup early-reflection layer ahead of the diffuse tail; **Size** continuously scales that layer's apparent dimensions, decoupled from Decay; **Early/Late Balance** crossfades between the early layer and the diffuse late tail.
- **Bass Decay** (25-175%) lets the low band ring longer (or shorter) than the mid/high bands, matching real-hall low-frequency-decay behavior.
- **Freeze** sustains the tail's current spectral content instead of letting it decay - a bounded, convolution-based "hold the pad" mode that structurally can't develop the dulling/periodicity artifacts feedback-loop-based infinite reverbs are documented to have.
...
Decay, Damping, Space, Size, Early/Late Balance, Bass Decay, and Freeze don't touch the audio-thread signal path directly - they drive a background, message-thread-only impulse-response regeneration step (see [`docs/architecture.md`](docs/architecture.md) for the full explanation of why, and how it stays real-time safe).
...
Full musical descriptions and usage tips: [`docs/manual.md`](docs/manual.md). Factory presets: [`docs/presets.md`](docs/presets.md).
